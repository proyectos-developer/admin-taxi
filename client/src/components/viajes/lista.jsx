...
import CardViaje from './cards/viaje.jsx'

export default function ListaViajes({proporcional}) {
    ...
    useEffect (() => {
        if (get_viajes_filtro_total && get_viajes_filtro_total.viajes && get_viajes_filtro_total.success === true){
            let data = get_viajes_filtro_total.viajes.length
            let lista = []
            ...
            <div className='d-flex' style={{width: 1480 / proporcional, height: 50 / proporcional, marginBottom: 25 / proporcional }}>
                ...
                <div className='d-flex' style={{width: 370 / proporcional, height: 50 / proporcional}}>
                    <p className='mb-0' 
                        ...
                        mostrando del {begin} al {begin + viajes.length} de {total_viajes} viajes
                    </p>
                </div>
                        ...
                        lista_viajes.map ((viaje, numviaje) => {
                            return (
                                <div key={numviaje} className='d-flex' 
                                    ...
                                    {
                                        viajes[(4 *  numviaje)] ? ( 
                                            <CardViaje viaje={viajes[(4 *  numviaje)]} key={(4 *  numviaje)} index={(4 *  numviaje)} proporcional={proporcional}/>
                                        ) : null
                                    }
                                    {
                                        viajes[(4 *  numviaje) + 1] ? ( 
                                            <CardViaje viaje={viajes[(4 *  numviaje) + 1]} key={(4 *  numviaje) + 1} index={(4 *  numviaje) + 1} proporcional={proporcional}/>
                                        ) : null
                                    }
                                    {
                                        viajes[(4 *  numviaje) + 2] ? ( 
                                            <CardViaje viaje={viajes[(4 *  numviaje) + 2]} key={(4 *  numviaje) + 2} index={(4 *  numviaje) + 2} proporcional={proporcional}/>
                                        ) : null
                                    }
                                    {
                                        viajes[(4 *  numviaje) + 3] ? ( 
                                            <CardViaje viaje={viajes[(4 *  numviaje) + 3]} key={(4 *  numviaje) + 3} index={(4 *  numviaje) + 3} proporcional={proporcional}/>
                                        ) : null
                                    }
                                </div>
                            )
