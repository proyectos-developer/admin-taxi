...
import { set_filtro_calificaciones_search_order_amount, set_limpiar_filtros } from '../../redux/actions/filtrosactions.js'

import clean from '../../iconos/clean_grey.png'
import left from '../../iconos/left_grey.png'
import right from '../../iconos/right_grey.png'

import CardCalificacionCell from './cards/calificacioncell.jsx'
...
export default function ListaCalificacionesCell({proporcional}) {
    ...
    const {get_calificaciones_filtro_total} = useSelector (({calificaciones}) => calificaciones)
    const {filtro_calificaciones_search_order_amount} = useSelector(({filtrosreducer}) => filtrosreducer)

    useEffect (() => {
        const id = filtro_calificaciones_search_order_amount.id
        const order_by = filtro_calificaciones_search_order_amount.order_by
        const order = filtro_calificaciones_search_order_amount.order
        const inicio = filtro_calificaciones_search_order_amount.begin
        const cantidad = filtro_calificaciones_search_order_amount.amount
        setBegin (inicio)
        setAmount (cantidad)
        dispatch(calificacionesdata(calificacionesConstants(id, order_by, order, inicio, cantidad, false).get_calificaciones_filtro_total))
    }, [])

    useEffect(() => {
        setFiltros(filtro_calificaciones_search_order_amount)
    }, [filtro_calificaciones_search_order_amount])

    useEffect (() => {
        if (get_calificaciones_filtro_total && get_calificaciones_filtro_total.calificaciones && get_calificaciones_filtro_total.success === true){
            if (get_calificaciones_filtro_total.total_calificaciones){setTotalCalificaciones(get_calificaciones_filtro_total.total_calificaciones)}
            setCalificaciones (get_calificaciones_filtro_total.calificaciones)
            setListaCalificaciones (get_calificaciones_filtro_total.calificaciones)
        }
    ...
    const ordenar_por_filtro = (value) => {
        ...
        dispatch (set_filtro_calificaciones_search_order_amount({pagina: 'calificaciones', id: id, order_by: order_by, order: order, begin: 0, amount: 16}))
        dispatch(calificacionesdata(calificacionesConstants(id, order_by, order, 0, 16, false).get_calificaciones_filtro_total))
    }

    const siguientes_calificaciones = () => {
        ...
        dispatch(set_filtro_calificaciones_search_order_amount({pagina: 'calificaciones', id: id, order_by: order_by, order: order, begin: inicio, amount: cantidad}))
        dispatch(calificacionesdata(calificacionesConstants(id, order_by, order, inicio, cantidad, false).get_calificaciones_filtro_total))
    }
    ...
    const limpiar_filtros = () => {
        setBegin (0)
        setOrderBy('')
        dispatch(set_filtro_calificaciones_search_order_amount({pagina: '', id: 0, search: 0, order_by: 0, order: 0, begin: 0, amount: 16}))
        dispatch (calificacionesdata (calificacionesConstants(0, 0, 0, 0, 0, 16, false).get_calificaciones_filtro_total))
        ...
        <div className='position-relative' style={{width: 499 / proporcional, paddingLeft: 20 / proporcional, paddingRight: 20 / proporcional}}>
            <div className='d-flex' style={{width: 459 / proporcional, height: 50 / proporcional, marginBottom: 25 / proporcional }}>
                ...
                <p className='mb-0' 
                    ...
                    del {begin} al {begin + calificaciones.length} de {total_calificaciones}
                </p>
                        ...
                        lista_calificaciones.map ((calificacion, numcal) => {
                            return (
                                <div key={numcal} className='d-flex' 
                                    ...
                                    <CardCalificacionCell calificacion={calificacion} key={numcal} index={(2 *  numcal) + 1} proporcional={proporcional}/>
                                </div>
                            )
                        })
                    ) : null
                ...
                <div className='d-flex justify-content-end' style={{width: 375 / proporcional, height: 50 / proporcional}}>
                    {
                        calificaciones.length > 0 && begin + amount < total_calificaciones  ? (
                            <img className='cursor-pointer' src={right} 
                                ...
                                onMouseOver={() => setBotonPosteriores(true)} onMouseLeave={() => setBotonPosteriores(false)} onClick={() => {siguientes_calificaciones(); setBotonPosteriores(false)}}/>
                        ) : null
                    }
                </div>
            </div>
